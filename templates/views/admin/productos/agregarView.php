  ...
  <?php flasher() ?>
	...
	<div class="row">
		<div class="offset-xl-3 col-xl-6 col-12">
          ...
          <form action="admin/productos-store" method="POST">
            <?php echo insert_inputs(); ?>
						...
						<div class="form-group row">
							...
							<div class="<?php echo bs_col([6,6,6,12,12]) ?>">
								...
								<input type="text" class="form-control" name="sku" id="sku" value="<?php echo randomPassword(12, 'numeric'); ?>">
							</div>
						</div>
						...
						<div class="form-group row">
							<div class="<?php echo bs_col([4,4,4,12,12]) ?>">
								...
								<input type="text" class="form-control money" name="precio" id="precio" placeholder="0.00" required>
							</div>
							<div class="<?php echo bs_col([4,4,4,12,12]) ?>">
								...
								<input type="number" class="form-control" min="1" max="999" name="capacidad" id="capacidad" placeholder="123" required>
							</div>
							<div class="<?php echo bs_col([4,4,4,12,12]) ?>">
								...
								<select class="form-control" name="tipo_servicio" id="tipo_servicio" required>
									<?php foreach (get_producto_tipos_servicio() as $ts): ?>
										<option value="<?php echo $ts[0]; ?>"><?php echo $ts[1]; ?></option>
									<?php endforeach; ?>
								</select>
							</div>
						</div>
							...
							<div class="<?php echo bs_col([4,4,4,12,12]) ?>">
								...
								<select class="form-control" name="tiempo_entrega" id="tiempo_entrega" required>
									<?php foreach (get_producto_tiempos_entrega() as $te): ?>
										<option value="<?php echo $te[0]; ?>"><?php echo $te[0]; ?></option>
									<?php endforeach; ?>
								</select>
							</div>
						</div>
